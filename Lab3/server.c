...
enum {
	SYN = 0x01,		//for sequential bit
	ACK = 0x02,		//for acknowledgement bit
	FIN = 0x04,		//for finish bit
};

int server_socket(int portno);	
int connect_to_client(int serv_sock, int portno);
...
int main(int argc, char **argv)
{
	...
	int serv_sock = server_socket(portno);
	int accept_sock = connect_to_client(serv_sock,portno);
...
//The function compute_cksum will compute the checksum for each packet
...
//function to recieve the close connection request
int recv_close_req(int accept_sock)
{
	int num_data_recv, num_sent, temp_portno;
	char buffer[255];
...
//function to print each TCP segment
void print_tcp_seg(struct tcp_hdr *tcp_seg)
{
	FILE *fp;

	fp = fopen("outputServ.txt", "a+");
...
//function to receive the connection close
int recv_close_ack(int accept_sock)
{
	int num_data_recv;
	char buffer[255];
