...
//Course: CSCE 3530
	...
	int connect_to_server(char *serverip, int portno);
	...
	int main(int argc, char **argv)
	{
	  ...
	  fp = fopen ("outputCli.txt", "w");
	  fclose(fp); 

		
		if (argc < 2) {
			printf("Usage %s <serverip> <Port_Number> \n", argv[0]);
			exit(0);
		}
	  serverip = argv[1];
	  portno = atoi(argv[2]);	//set port number from user input
	  sockfd = connect_to_server(serverip, portno); //connect to server
	...
	int connect_to_server(char *serverip, int portno)
	{
	  ...
	  serv_addr.sin_addr.s_addr=inet_addr(serverip); //IP for cse01.cse.unt.edu
